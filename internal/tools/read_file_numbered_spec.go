...
func (s *ReadFileNumberedSpec) Description() string {
	return fmt.Sprintf("Read a file from the filesystem (format: [padded line number][space][line]). Can read entire file or multiple specific line ranges using the sections parameter (max %d lines).", consts.MaxLinesPerRead)
}
